...
def plot_log(logfile_path, output_dir, train_dict, test_dict):
    st = 1
    num = len(train_dict)
    train_num_iters = [train_dict[i]['NumIters'] for i in range(num)]
    train_los_ids = [train_dict[i]['loss_ids'] for i in range(num)]
    train_acc_ids = [train_dict[i]['accuracy_ids'] for i in range(num)]

    num = len(test_dict)
    test_num_iters = [test_dict[i]['NumIters'] for i in range(num)]
    test_los_ids = [test_dict[i]['loss_ids'] for i in range(num)]
    test_acc_ids = [test_dict[i]['accuracy_ids'] for i in range(num)]

    fig = plt.figure(1)
    plt.subplot(211)
    plt.plot(train_num_iters[st:], train_los_ids[st:], 'b*-', label='train loss')
    plt.plot(test_num_iters[st:], test_los_ids[st:], 'ro-', label='test loss')
    plt.legend(loc='upper center', ncol=2)
    plt.subplot(212)
    plt.plot(train_num_iters[st:], train_acc_ids[st:], 'b*-', label='train acc') 
    plt.plot(test_num_iters[st:], test_acc_ids[st:], 'ro-', label='test acc')
